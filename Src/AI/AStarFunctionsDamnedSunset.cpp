/**
@file AStarFunctionsDamnedSunset.cpp
...
namespace AI 
{
	...
	Constructor
	*/
	CAStarFunctionsDamnedSunset::CAStarFunctionsDamnedSunset(void)
	{
	}
	...
	*/
	...
	entre los puntos.
	*/
	float CAStarFunctionsDamnedSunset::LeastCostEstimate( void* stateStart, void* stateEnd )
	{
		// Función heurística para A*.
		// En el caso de Damned Sunset, una heurística admisible es la distancia entre los nodos 
		...
		CGridmapGraph* navMap = CServer::getSingletonPtr()->getNavigationMap();
		Vector3 orig = navMap->getNodePosition(idOrigen);
		Vector3 dest = navMap->getNodePosition(idDestino);
	...
	*/	
	void CAStarFunctionsDamnedSunset::AdjacentCost( void* state, std::vector< micropather::StateCost > *adjacent )
	{
		...
		// Utilizamos el grafo de navegación:
		CGridmapGraph* navMap = CServer::getSingletonPtr()->getNavigationMap();
		std::list<unsigned int> neighbours = navMap->getNeighbours(idNodo);
		// Llenamos la lista de adyacentes con pares de (nodo vecino, coste en llegar)
		for (std::list<unsigned int>::iterator it = neighbours.begin(); it != neighbours.end(); it++) {
			micropather::StateCost nodeCost = {(void*)(*it), navMap->getCost((int) state, (*it))};
	...
	*/
	void  CAStarFunctionsDamnedSunset::PrintStateInfo( void* state )
	{
		CGridmapGraph* navMap = CServer::getSingletonPtr()->getNavigationMap();
		Vector3 position = navMap->getNodePosition((int) state);
