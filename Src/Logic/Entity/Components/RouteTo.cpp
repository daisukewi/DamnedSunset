	...
	void CRouteTo::tick(unsigned int msecs)
	{
		...
		if (_recalculateRoute) {
			...
			if (_currentRoute == 0) {
				...
				sendMoveMessage((*_currentRoute)[_currentNode], AI::IMovement::MOVEMENT_KINEMATIC_AVOID, true);
			}
		}
		...
		if (!_arrived) {
			if (_nextWaypoint) {
				...
				//	sendMoveMessage((*_currentRoute)[_currentNode], AI::IMovement::MOVEMENT_DYNAMIC_ARRIVE);
					...
					sendMoveMessage((*_currentRoute)[_currentNode], AI::IMovement::MOVEMENT_KINEMATIC_AVOID);
				}
			}
		}
	} // tick
