...
public class Map implements Serializable, ExportableData {
	...
	public Map(int width, int height, CellType basicCell) {
		...
		// Inicialización del mapa con la celda básica pasada como parámetro.
		_map = new Cell[_mapWidth][_mapHeight];
		for (int x = 0; x < _mapWidth; x++)
			for (int y = 0; y < _mapHeight; y++)
	...
	private void closeElement() {
		...
		// Si tengo parámetros para importar en este punto es porque he cerrado una entidad y ya no hay mas parámetros para leer
		// por lo tanto escribo los parámetros leídos.
	...
	private void checkBeginElement(Vector<CellType> cells, String[] s) {
		...
		// Si es definición de nombre de celda me la guardo para empezar a leer la información de la celda.
		if (index != -1)
			_importingCell = s[0];
		else {
			
			// Si en este punto leo una apertura de llave después de un nombre es porque lo que viene ahora es una
			// definición de entidad y el nombre es el parámetro name de la misma.
			if (s[2].equals("{")) {
	...
	private void extractCellParameter(String[] s) {
		...
		if (s[0].equals(CELL_POSITION)) {
			...
			// Después de actualizar la posición actualizo también el máximo valor de x e y acumulado
			// para redimensionar al final el tamaño del mapa.
	...
	private void extractLongEntityParameter(String[] s) {
		...
		// Actualizo el valor de la posición y el tipo de la entidad para poder construirla.
...
*/
	...
	public void importData(String data, int type, Vector<CellType> cells, Vector<ColorEntity> entities) {
		...
		if (type == 0) {
			...
			// Compruebo si estoy importando o estoy al principio de la sección del archivo que me importa.
			if ((data.contains("Map")) || _importing) {
				...
				// Compruebo que no estoy al principio y voy a leer datos válidos.
				if (!data.contains("Map")) {
					...
					// Si me encuentro con un cierre de llave en esta altura es porque se cierra una sección de celda o de entidad.
					if (data.startsWith("}"))
						closeElement();
					
					// Miro si la cadena leída es de longitud 3 porque podría ser una definición de nombre de celda o entidad
					// o una definición de parámetro de entidad.
					if ((s != null) && (s.length == 3)) {
						...
						// un parámetro de la entidad actual. Me la guardo, pues.
						if ((_importingEntity != null) && (!s[2].equals("{")))
							extractShortEntityParameter(s);
						
					}
					else
						// Si la longitud de la cadena leída es mayor que 6 es porque estoy leyendo un parámetro de entidad o de celda.
						if ((s != null) && (s.length > 3))
							// Si estoy leyendo una celda actualizo el información de la misma.
							if (_importingCell != null)
								extractCellParameter(s);
							else
								// Si estoy leyendo una entidad me guardo el parámetro.
